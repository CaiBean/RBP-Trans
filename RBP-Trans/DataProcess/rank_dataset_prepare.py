...
h5_file = '/share/home/xuls/tongbu/Reformer/data/encode_eclip_bc.h5'
...
for mode in ['trn','val', 'test']:
    ...
    label = list(df[f'{mode}_label'])
    RBP, cellType = zip(*[ele.decode().split('_') for ele in df[f'{mode}_prefix']])
    dataTable = pd.DataFrame({'seq': sequence, 'label': np.array(label).tolist(), 'RBP': RBP, 'cellType': cellType})
...
final_df.to_parquet('test.parquet')  # 修正拼写
print("保存完成")